...
export function validateRelease(b){
 const out={};
 for(const [key,max] of [['version',60],['title',160],['notes',6000]]){if(typeof b[key]!=='string'||!b[key].trim()||(key==='notes'?richLength(b[key]):b[key].length)>max)fail('Enter a valid '+key,400);out[key]=b[key].trim();}
 ...
 const code=b.downloader_code===undefined?'':b.downloader_code;
 if(typeof code!=='string'||(code.trim()!==''&&!/^\d{1,12}$/.test(code.trim())))fail('Downloader code must contain only digits, or be blank.',400);
 out.downloader_code=code.trim();
...
export async function releaseAdmin(request,env,auth,{json,safeJson}){
 ...
 const data=validateRelease({...b,downloader_code:b.downloader_code===undefined?(old.downloader_code||''):b.downloader_code});
