...
export async function accountUpdateReport(env,auth){
 ...
 return {target,generated_at:t,scope:scoped?'All accounts':'Only accounts assigned to this administrator',users:users.map(u=>{
  ...
  const updated=builds.some(b=>b===27||b===28),old=builds.some(b=>b<target);
...
export function formatAccountUpdateReport(r){
 const groups=[['TEST BUILD REPORTED',u=>u.last_build===28],['CURRENT PUBLIC RELEASE REPORTED',u=>u.last_build===27],['OLDER VERSION LAST REPORTED',u=>u.last_build&&u.last_build<27],['UNRECOGNIZED NEWER VERSION',u=>u.last_build>28],['UNKNOWN VERSION',u=>!u.last_build]];
