import {accountUpdateReport,formatAccountUpdateReport} from './account-update-report.js';
import {telegramTransport} from './bot-delivery.js';
...
export async function usageCommand(env,id,cmd,s){
 ...
 if(env.BOT_GROUP_REPLY)return send(env,id,'Open this report privately. Usage information is only available to authorized administrators.',{inline_keyboard:[[{text:'📊 Open private report',url:'https://t.me/'+s.bot_username+'?start='+ (cmd==='website_release'?'website_release':cmd==='update_report'?'update_report':'usage')}]]});
 ...
 if(cmd==='update_report'){
  const data=await accountUpdateReport(env,auth),text=formatAccountUpdateReport(data);
 ...
 const links=[['📄 Download Update & Link Report','update_report'],['🟢 Online Now','usage:online:'+period],['🔗 Linked Accounts','usage:links:'+period],['🏆 Top 10 Watch Time','usage:most:'+period],['⌛ Least Watch Time','usage:least:'+period],['📱 Most App Visits','usage:visits:'+period],['🔑 Most App Sign-ins','usage:signins:'+period]];
