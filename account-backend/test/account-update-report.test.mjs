...
test('report distinguishes current, old, mixed and unknown with verified links',async()=>{
 ...
 for(const [name,build] of [['updated',28],['updated',27],['older',27]])await f.request('/me/activity',{method:'POST',token:f.token(name),body:{sequence:1,mode:'idle',playing:false,build}});
 ...
 assert.match(formatAccountUpdateReport(r),/UPDATED AND LINKED \(1\)/);
