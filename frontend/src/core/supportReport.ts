...
export function supportReport(version:string,versionCode:number,raw:string):string {
 ...
 return JSON.stringify({app:'Charming MediaLab',version:version.replace(/[^a-zA-Z0-9._-]/g,'').slice(0,64),versionCode,createdAt:new Date().toISOString(),playback},null,2);
}
