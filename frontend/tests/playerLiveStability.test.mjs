...
test("Media3 uses the hardened live-TV buffers and bounded native recovery policy", async () => {
  ...
  assert.match(native, /MIN_BUFFER_MS_LOW_RAM = 10_000/); assert.match(native, /MAX_BUFFER_MS_LOW_RAM = 30_000/); assert.match(native, /PLAYBACK_BUFFER_MS_LOW_RAM = 1_500/); assert.match(native, /REBUFFER_BUFFER_MS_LOW_RAM = 2_500/); assert.match(native, /TARGET_BUFFER_BYTES_LOW_RAM = 16 \* 1024 \* 1024/); assert.match(native, /MIN_BUFFER_MS_NORMAL = 15_000/); assert.match(native, /MAX_BUFFER_MS_NORMAL = 60_000/); assert.match(native, /PLAYBACK_BUFFER_MS_NORMAL = 1_500/); assert.match(native, /REBUFFER_BUFFER_MS_NORMAL = 2_500/); assert.match(native, /TARGET_BUFFER_BYTES_NORMAL = 48 \* 1024 \* 1024/); assert.match(native, /CharmMemoryCoordinator\.budgets\(\)\.lowRam/); assert.match(native, /HUNG_BUFFER_REPREPARE_MS = 12_000L/); assert.match(native, /TRANSPORT_HUNG_BUFFER_REPREPARE_MS = 16_000L/); assert.match(native, /MAX_AUTO_RECOVERIES = 4/); assert.match(native, /RECOVERY_BACKOFF_MS = longArrayOf\(0L, 1_000L, 3_000L, 6_000L\)/); assert.match(native, /readTimeout\(10, TimeUnit\.SECONDS\)/); assert.match(native, /recoveryAttempts >= MAX_AUTO_RECOVERIES/); assert.match(native, /instance\.prepare\(\)/);
});
