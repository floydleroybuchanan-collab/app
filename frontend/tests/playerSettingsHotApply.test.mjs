...
test("real-device defaults recommend VLC for live while keeping Media3 available", async () => {
  ...
  assert.match(settings, /label="Player engine"/); assert.match(settings, /VLC \(recommended for live\)/); assert.match(settings, /Media3/); assert.match(settings, /engine === "vlc"|playerEngine === "vlc"/); assert.match(settings, /VLC hardware decoding/); assert.match(settings, /VLC audio output/);
