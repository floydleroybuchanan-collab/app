...
test("locked Media3 safety budgets remain unchanged", async () => {
  ...
  for (const marker of ["MIN_BUFFER_MS_LOW_RAM = 1_000","MAX_BUFFER_MS_LOW_RAM = 2_500","PLAYBACK_BUFFER_MS_LOW_RAM = 500","REBUFFER_BUFFER_MS_LOW_RAM = 1_000","MIN_BUFFER_MS_NORMAL = 1_000","MAX_BUFFER_MS_NORMAL = 2_500","PLAYBACK_BUFFER_MS_NORMAL = 500","REBUFFER_BUFFER_MS_NORMAL = 1_000","HUNG_BUFFER_REPREPARE_MS = 5_000L","TRANSPORT_HUNG_BUFFER_REPREPARE_MS = 7_000L","MAX_AUTO_RECOVERIES = 4","longArrayOf(0L, 1_000L, 2_000L, 4_000L)"]) assert.match(manager, new RegExp(marker.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
});
