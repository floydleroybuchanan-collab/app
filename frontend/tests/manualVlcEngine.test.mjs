...
test("locked Media3 safety budgets remain unchanged", async () => {
  ...
  for (const marker of ["MIN_BUFFER_MS_LOW_RAM = 10_000","MAX_BUFFER_MS_LOW_RAM = 30_000","PLAYBACK_BUFFER_MS_LOW_RAM = 1_500","REBUFFER_BUFFER_MS_LOW_RAM = 2_500","MIN_BUFFER_MS_NORMAL = 15_000","MAX_BUFFER_MS_NORMAL = 60_000","PLAYBACK_BUFFER_MS_NORMAL = 1_500","REBUFFER_BUFFER_MS_NORMAL = 2_500","HUNG_BUFFER_REPREPARE_MS = 12_000L","TRANSPORT_HUNG_BUFFER_REPREPARE_MS = 16_000L","MAX_AUTO_RECOVERIES = 4","longArrayOf(0L, 1_000L, 3_000L, 6_000L)"]) assert.match(manager, new RegExp(marker.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
});
