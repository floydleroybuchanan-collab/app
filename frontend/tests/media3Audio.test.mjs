...
test("audio diagnostics fingerprint never retains raw URI", async () => { const { fingerprintStreamUri, recordAudioDiagnostics, audioDiagnosticsExtras } = await import("../src/core/audioDiagnostics.ts"); const uri = "http://example.test/live/secret-token-abc/index.m3u8|User-Agent=VLC"; const key = fingerprintStreamUri(uri, "hls"); assert.match(key, /^hls:/); assert.doesNotMatch(key, /secret-token|index\.m3u8/); const snap = recordAudioDiagnostics({ engine: "media3", role: "fullscreen", streamKey: key, trackId: "g0:t0:audio/ac3:eng:384000", mimeType: "audio/ac3", decoder: "ffmpeg-ac3", language: "eng", isSupported: true, trackCount: 1, supportedCount: 1, selectedBy: "auto-supported" }); const extras = audioDiagnosticsExtras(snap); assert.equal(extras.audioMime, "audio/ac3"); assert.equal(extras.audioDecoder, "ffmpeg-ac3"); assert.equal(extras.audioSupported, true); });
