...
test("known TS/HLS/DASH paths bypass opaque routing and keep the locked playback budgets", async () => {
  ...
  assert.match(manager, /MIN_BUFFER_MS_LOW_RAM = 15_000/);
  assert.match(manager, /MAX_BUFFER_MS_LOW_RAM = 45_000/);
  assert.match(manager, /PLAYBACK_BUFFER_MS_LOW_RAM = 4_000/);
  assert.match(manager, /REBUFFER_BUFFER_MS_LOW_RAM = 8_000/);
  assert.match(manager, /MIN_BUFFER_MS_NORMAL = 15_000/);
  assert.match(manager, /MAX_BUFFER_MS_NORMAL = 45_000/);
  assert.match(manager, /PLAYBACK_BUFFER_MS_NORMAL = 4_000/);
  assert.match(manager, /REBUFFER_BUFFER_MS_NORMAL = 8_000/);
  assert.match(manager, /readTimeout\(30, TimeUnit\.SECONDS\)/);
