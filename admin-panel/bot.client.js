 ...
 if(admin.is_owner){
  ...
  finder.append(button('Find my Telegram group',async()=>{
   ...
   try{
    ...
    if(!r.groups.length){status.textContent=r.message||'No group found yet. In Telegram, make @CharmIPTVAssistantBot a group administrator, then send Mr Charm Help in that group and click Find my Telegram group again.';return;}
...
async function botSupportAdmins(box){
 box.append(button('Find human Telegram admins',async()=>{await api('/admin/bot/support-admins/sync','POST',{});await loadBot();},'primary'),el('p','Choose which people appear in Contact an Admin. Every enabled current human admin gets a button. Without a public username, their button shows contact instructions. Bots are excluded.','help'));
 const d=await api('/admin/bot/support-admins');
 box.append(el('p',d.admins.filter(a=>a.enabled).length+' support admins enabled. Telegram group membership is checked when members open the list.'));
 for(const a of d.admins){
  const card=el('div',undefined,'card');card.append(el('strong',a.name),el('p','Telegram ID: '+a.telegram_id),el('p','Last username from Telegram: '+(a.username?'@'+a.username:'Not set')));
  if(a.app_username)card.append(el('p','App administrator: '+a.app_username+(a.account_contact_username?' · @'+a.account_contact_username:'')));
  const username=field(card,'Contact username (optional; @name)','contact_username',a.contact_username||'','text',{maxLength:33,autocomplete:'off'});
  card.append(el('p','Current Telegram usernames take priority. This saved username is a fallback when Telegram supplies none. It must be their real public @username.','help'));
