...
const isRich=s=>typeof s==='string'&&s.startsWith(PREFIX);
function readRich(value){
...
const richPlain=s=>readRich(s).text;
const richLength=s=>richPlain(s).length;
const richAppend=(s,suffix)=>isRich(s)?PREFIX+readRich(s).html+'<p>'+escape(suffix)+'</p>':s+suffix;
const richPack=html=>{const value=PREFIX+html;readRich(value);return value;};


window.CharmRich={isRich,readRich,richPlain,richLength,richAppend,richPack,escape};
})();
